...
//查询存储引擎查询表备注信息
show full columns from usr_user;
...
//一个sql返回多个总数
select count(*) status_num, count(case when status = 1 then status end) status_1_num, count(case when status = 2 then status end) status_2_num from table_name;
